...
async function loadResultsPanel() {
  ...
  try {
    ...
    // Aplicar resolución de estados programados
    AppState.elections = (electionsRes.ok ? electionsRes.data : [])
      .map(e => typeof resolveElectionState==='function' ? resolveElectionState(e) : e);
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px;flex-wrap:wrap;gap:12px">
        ...
        <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
          <select id="results-election-select" class="form-input form-select" style="max-width:280px" onchange="loadElectionResults(this.value)">
            ...
            ${elections.map(e=>{
              const e2=typeof resolveElectionState==='function'?resolveElectionState(e):e;
              const icon={active:'🟢',ended:'🔴',paused:'🟡',scheduled:'🔵',inactive:'⚪'}[e2.estado]||'⚪';
              return `<option value="${e2.id}">${icon} ${sanitize(e2.nombre)} — ${statusLabel(e2.estado)}</option>`;
            }).join('')}
          </select>
...
function renderResultsData(data) {
  ...
  // Resolver estado de la elección para mostrar en resultados
  const elecResuelta = typeof resolveElectionState==='function' ? resolveElectionState(election) : election;

  cont.innerHTML = `
    <!-- Estado de la elección -->
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;padding:12px 16px;background:var(--bg-soft);border-radius:10px;border:1px solid var(--border)">
      <span style="font-size:.88rem;color:var(--text-secondary)">Estado de la votación:</span>
      ${statusBadge(elecResuelta.estado)}
      <span style="font-size:.78rem;color:var(--text-light);margin-left:auto">
        Los datos "Registrados" y "Sin votar" incluyen solo votantes de secciones asignadas a esta votación
      </span>
    </div>
    <!-- Stats generales -->
    ...
    <div style="font-size:.74rem;color:var(--text-light);text-align:right;margin-top:-16px;margin-bottom:20px">
      📌 Registrados y sin votar = solo votantes de secciones habilitadas para esta votación
    </div>
