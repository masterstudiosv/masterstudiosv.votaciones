...
async function verifyVoterID() {
  ...
  try {
    ...
    // Verificar secciones permitidas de la urna
    const urna = AppState.currentUrna;
    if (urna?.seccionesPermitidas) {
      const perms = urna.seccionesPermitidas.split(',').filter(Boolean);
      if (perms.length > 0 && voter.seccionId && !perms.includes(voter.seccionId)) {
        errEl.textContent = 'Este votante no puede votar en esta urna (sección no permitida).';
        errEl.style.display = 'flex';
        if (input) input.value = '';
        return;
      }
...
async function loadUrnaPanel() {
  ...
  try {
    ...
    // Buscar votación activa (o la asignada a esta urna)
    const elections = electionsRes.ok ? electionsRes.data : [];
    const urna = AppState.currentUrna;

    let activeElection = null;
    if (urna?.eleccionId) {
      // Primero buscar la votación específica de esta urna
      activeElection = elections.find(e => e.id === urna.eleccionId && ['active','scheduled','paused'].includes(e.estado));
    }
    if (!activeElection) {
      // Si no tiene asignada, buscar cualquier activa
    ...
    if (urna) {
      ...
      if(ne) ne.textContent = urna.nombre||'';
      if(le) le.textContent = urna.ubicacion||'';
      if(se) se.textContent = urna.sector||'';
    }
