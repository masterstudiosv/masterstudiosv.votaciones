...
function renderAdminDashboard() {
  ...
  if (!cont) return;
  // Aplicar resolución de estados programados antes de mostrar
  const elections = (AppState.elections || []).map(e => typeof resolveElectionState === 'function' ? resolveElectionState(e) : e);
...
async function saveElection(id='') {
  ...
  try{
    ...
    if(res.ok){
      ...
      const fresh=await API.getElections();
      if(fresh.ok) AppState.elections=fresh.data.map(e=>typeof resolveElectionState==='function'?resolveElectionState(e):e);
      renderElectionsMgmt();
...
function openElectionControl(id) {
  const eRaw=(AppState.elections||[]).find(x=>x.id===id); if(!eRaw) return;
  const e = typeof resolveElectionState === 'function' ? resolveElectionState(eRaw) : eRaw;
  const m=document.createElement('div');m.className='modal-overlay';m.id='election-control-modal';
  // Formatear horas programadas para mostrar en el modal
  const startFmt = e.inicioProgramado ? formatTimeSV(e.inicioProgramado) : '—';
  const endFmt   = e.finProgramado    ? formatTimeSV(e.finProgramado)    : '—';
  // Convertir a formato datetime-local (YYYY-MM-DDTHH:mm) para el input
  function toDatetimeLocal(v) {
    ...
    try {
      ...
      return sv.getUTCFullYear()+'-'+pad(sv.getUTCMonth()+1)+'-'+pad(sv.getUTCDate())+'T'+pad(sv.getUTCHours())+':'+pad(sv.getUTCMinutes());
    } catch(ex){ return String(v).slice(0,16).replace(' ','T'); }
  }
  m.innerHTML=`<div class="modal-box">
    ...
    <div class="modal-body">
      ...
      ${e.inicioProgramado?`<div style="font-size:.78rem;color:var(--text-secondary);margin-bottom:16px">⏰ Inicio: <strong>${startFmt}</strong>${e.finProgramado?' &nbsp;|&nbsp; 🏁 Fin: <strong>'+endFmt+'</strong>':''}</div>`:''}
      ...
      ${e.estado==='ended'?
        '<div class="alert alert-warning" style="margin:12px 0">🔒 <strong>Votación finalizada.</strong> No se puede reactivar, pausar, reiniciar ni reprogramar.</div>'
        ...
        <div style="border-top:1px solid var(--border);padding-top:14px;margin-top:14px">
          <div style="font-size:.78rem;color:var(--text-secondary);margin-bottom:10px">📅 Programar inicio/fin automático (hora El Salvador)</div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
            <div><label class="form-label">Inicio programado</label>
              <input type="datetime-local" id="ctrl-start" class="form-input" value="${toDatetimeLocal(e.inicioProgramado)}"></div>
            <div><label class="form-label">Fin programado</label>
              <input type="datetime-local" id="ctrl-end" class="form-input" value="${toDatetimeLocal(e.finProgramado)}"></div>
          </div>
...
async function doElectionControl(id,estado) {
  ...
  try{
    ...
    if(res.ok){
      const fresh=await API.getElections();
      if(fresh.ok){
        // Aplicar resolución de estados programados al actualizar
        AppState.elections = fresh.data.map(e => typeof resolveElectionState === 'function' ? resolveElectionState(e) : e);
      }
      el('election-control-modal')?.remove(); renderElectionsMgmt(); renderAdminDashboard(); showToast('Estado actualizado: '+estado,'success');
    } else{errEl.textContent=res.error||'Error';errEl.style.display='flex';}
  }catch(err){errEl.textContent='Error: '+err.message;errEl.style.display='flex';}
  finally{hideLoading();}
}
...
async function doToggleResults(id,mostrar) {
  ...
  try{
    ...
    if(res.ok){
      const fresh=await API.getElections();
      if(fresh.ok) AppState.elections=fresh.data.map(e=>typeof resolveElectionState==='function'?resolveElectionState(e):e);
      el('election-control-modal')?.remove(); renderElectionsMgmt(); renderAdminDashboard(); showToast(mostrar?'Resultados visibles':'Resultados ocultos','success');
    } else{errEl.textContent=res.error||'Error';errEl.style.display='flex';}
  }catch(err){errEl.textContent='Error: '+err.message;errEl.style.display='flex';}
  finally{hideLoading();}
}
...
async function openElectionConfig(id) {
  ...
  // Si la votación ya finalizó, mostrar en modo solo lectura
  const esFinalizaada = e.estado==='ended';
  ...
  m.innerHTML=`<div class="modal-box" style="max-width:700px;max-height:90vh">
    <div class="modal-header">
      <h3 class="modal-title">${esFinalizaada?'🔒 Ver':'⚙️ Configurar'}: ${sanitize(e.nombre)}</h3>
    ...
    <div class="modal-body" style="overflow-y:auto;max-height:75vh">
      ${esFinalizaada?'<div class="alert alert-warning" style="margin-bottom:14px">🔒 <strong>Votación finalizada.</strong> No se puede modificar candidatos ni cargos.</div>':''}
      <div class="tabs">
...
function renderPositionsTab() {
  ...
  const elecFinalizada = AppState.currentElection?.estado==='ended';
  cont.innerHTML=`<div style="display:flex;justify-content:flex-end;margin-bottom:12px">
    ${elecFinalizada?'<span class="badge badge-inactive">🔒 Solo lectura</span>'
      ...
      <tbody>${positions.length===0?'<tr><td colspan="3" style="text-align:center;padding:24px;color:var(--text-light)">No hay cargos creados</td></tr>':
        positions.map(p=>`<tr>
          ...
          <td><div style="display:flex;gap:6px">
            ${elecFinalizada?'<span style="font-size:.75rem;color:var(--text-light)">—</span>':
...
async function savePosition(id='') {
  ...
  showLoading('Guardando...');
  try{
    const res=await API.savePosition({id:id||null,eleccionId:AppState.currentElection.id,nombre,orden,usuario:'ADMIN'});
    if(res.ok){
...
async function deletePosition(id) {
  showConfirmModal('Eliminar Cargo','¿Eliminar este cargo?',async()=>{
    ...
    try{
      await API.deletePosition({id, eleccionId:AppState.currentElection?.id, usuario:'ADMIN'});
      const fresh=await API.getPositions(AppState.currentElection.id); if(fresh.ok) AppState.positions=fresh.data;
...
function renderCandidatesTab() {
  ...
  const positions=AppState.positions||[];
  const elecFinalizadaCand = AppState.currentElection?.estado==='ended';
  cont.innerHTML=`<div style="display:flex;justify-content:flex-end;margin-bottom:12px">
    ${elecFinalizadaCand?'<span class="badge badge-inactive">🔒 Solo lectura</span>'
    ...
    <div class="table-wrap"><table>
      ...
      <tbody>${candidates.length===0?'<tr><td colspan="5" style="text-align:center;padding:24px;color:var(--text-light)">No hay candidatos</td></tr>':
        candidates.map(c=>`<tr>
          ...
          <td><div style="display:flex;gap:6px">
            ${elecFinalizadaCand?'<span style="font-size:.75rem;color:var(--text-light)">—</span>':
...
async function saveCandidateAdmin(id='') {
  ...
  showLoading('Guardando...');
  try{
    const res=await API.saveCandidate({id:id||null,eleccionId:AppState.currentElection.id,posicionId:posId,posicionNombre:posNombre,nombre,seccion,fotoUrl,usuario:'ADMIN'});
    if(res.ok){
...
async function deleteCandidateAdmin(id) {
  showConfirmModal('Eliminar Candidato','¿Eliminar este candidato?',async()=>{
    showLoading('Eliminando...');
    try{
      await API.deleteCandidate({id, eleccionId:AppState.currentElection?.id, usuario:'ADMIN'});
...
function renderVotersMgmt() {
  ...
  cont.innerHTML=`
    ...
    ${voters.length===0?'<div class="alert alert-info" style="margin-bottom:12px"><span class="spinner" style="width:14px;height:14px;border-width:2px;display:inline-block;vertical-align:middle;margin-right:8px"></span>Cargando padrón en segundo plano... Si ya cargó, usa el botón de recargar.</div>':''}
    <div style="display:flex;gap:12px;margin-bottom:16px;flex-wrap:wrap;align-items:center">
      ...
      <button class="btn btn-ghost btn-sm" onclick="reloadVoters()" title="Recargar padrón">🔄</button>
    </div>
    <div class="table-wrap"><table>
...
async function reloadVoters() {
  showLoading('Actualizando padrón...');
  try {
    const res = await API.getVoters({});
    if (res.ok) { AppState.voters = res.data; renderVotersMgmt(); showToast('Padrón actualizado: '+res.data.length+' votantes','success'); }
    else showToast('Error al recargar padrón','error');
  } catch(err) { showToast('Error: '+err.message,'error'); }
  finally { hideLoading(); }
}
...
// Helper: obtener la eleccionId de un votante (basada en su sección)
function getEleccionIdDeVotante(v) {
  if (!v || !v.seccionId) return '';
  const sec = (AppState.sections||[]).find(s=>s.id===v.seccionId);
  return sec && sec.eleccionIds && sec.eleccionIds.length > 0 ? sec.eleccionIds[0] : '';
}

function openVoterModal(id=null) {
  // Si es nuevo votante, exigir que el admin elija una votación primero
  if (!id && !AppState.votersElectionId) {
    showToast('Primero selecciona una votación arriba antes de agregar votantes','warning',5000);
    return;
  }
  const v=id?(AppState.voters||[]).find(x=>x.id===id):null;
  // Para el modal mostrar solo secciones de la votación seleccionada (o la del votante)
  const eleccionIdParaModal = AppState.votersElectionId || (v ? getEleccionIdDeVotante(v) : '');
  const todasSecciones = AppState.sections || [];
  const seccionesFiltradas = eleccionIdParaModal
    ? todasSecciones.filter(s => (s.eleccionIds||[]).includes(eleccionIdParaModal))
    : todasSecciones;
  const secOpts = seccionesFiltradas.map(s=>`<option value="${s.id}" data-nombre="${sanitize(s.nombre)}" ${v&&v.seccionId===s.id?'selected':''}>${sanitize(s.nombre)}</option>`).join('');
  // Buscar tipoId de la votación seleccionada
  const eleccionActual = (AppState.elections||[]).find(e=>e.id===eleccionIdParaModal);
  const idLabel = eleccionActual?.tipoId || (AppState.currentElection?.tipoId) || 'NIE/DUI';
...
async function saveVoter(id='') {
  ...
  try{
    const res=await API.saveVoter({id:id||null,identificador,apellidos,nombres,seccionId,seccionNombre,
      eleccionId:AppState.votersElectionId||'', usuario:'ADMIN'});
    if(res.ok){
...
async function deleteVoter(id) {
  showConfirmModal('Eliminar Votante','¿Eliminar este votante del padrón?',async()=>{
    showLoading('Eliminando...');
    try{
      const res=await API.deleteVoter({id, eleccionId:AppState.votersElectionId||'', usuario:'ADMIN'});
...
async function doImportVoters() {
  ...
  try{
    const res=await API.importVoters({voters:withSection, eleccionId:AppState.votersElectionId||'', usuario:'ADMIN'});
    if(res.ok){
      const fresh=await API.getVoters({}); if(fresh.ok) AppState.voters=fresh.data;
...
async function exportVotersPDF(electionId) {
  ...
  showToast('Si no abre el PDF, permite las ventanas emergentes en la barra de dirección','info',6000);
  showLoading('Generando reporte...');
  try{
    const [resultsRes,votersRes,sectionsRes]=await Promise.all([
      API.getResults(electionId), API.getVoters({}), API.getSections()
    ]);
    if(!resultsRes.ok){showToast('Error al obtener resultados','error');return;}
    const data=resultsRes.data, election=data.election;
    const allVoters=votersRes.ok?votersRes.data:(AppState.voters||[]);
    const sections=sectionsRes.ok?sectionsRes.data:(AppState.sections||[]);

    // Mapa id->nombre de sección (nombre real, no fecha)
    const secMap={};
    sections.forEach(s=>{ secMap[s.id]=s.nombre; });

    // Filtrar solo votantes cuya sección pertenece a ESTA votación
    const seccionesDeEstaVotacion=new Set(
      sections.filter(s=>(s.eleccionIds||[]).includes(electionId)).map(s=>s.id)
    );
    const filteredVoters = seccionesDeEstaVotacion.size > 0
      ? allVoters.filter(v=>seccionesDeEstaVotacion.has(v.seccionId))
      : allVoters;

    const votedIDSet=new Set((data.votedIDs||[]).map(id=>String(id).toUpperCase()));
    const voted    = filteredVoters.filter(v=>votedIDSet.has(String(v.identificador).toUpperCase()));
    const notVoted = filteredVoters.filter(v=>!votedIDSet.has(String(v.identificador).toUpperCase()));

    function getSecNombre(v) {
      if (v.seccionId && secMap[v.seccionId]) return secMap[v.seccionId];
      const sn=String(v.seccionNombre||'');
      if (/^\d{2}\/\d{2}\/\d{4}/.test(sn)||/^\d{4}-\d{2}/.test(sn)) return '—';
      return sn||'—';
    }

    const now=new Date().toLocaleString('es-SV',{timeZone:'America/El_Salvador',hour12:false,year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'});
    const pct=filteredVoters.length>0?((voted.length/filteredVoters.length)*100).toFixed(1):0;
    const html=`<h1>Lista de Participacion - ${sanitize(election.nombre)}</h1>
      <p><strong>Generado:</strong> ${now}</p>
      <div style="margin:16px 0">
        <div class="stat-box"><div class="stat-val">${filteredVoters.length}</div><div class="stat-lbl">Registrados</div></div>
        ...
        <div class="stat-box"><div class="stat-val">${pct}%</div><div class="stat-lbl">Participacion</div></div>
      </div>
      <h2>SI votaron (${voted.length})</h2>
      <table><thead><tr><th>Identificador</th><th>Apellidos</th><th>Nombres</th><th>Seccion</th></tr></thead>
      <tbody>${voted.map(v=>`<tr><td>${sanitize(v.identificador)}</td><td>${sanitize(v.apellidos)}</td><td>${sanitize(v.nombres)}</td><td>${sanitize(getSecNombre(v))}</td></tr>`).join('')||'<tr><td colspan="4" style="text-align:center">Sin registros</td></tr>'}
      </tbody></table>
      <h2 style="margin-top:20px">NO votaron (${notVoted.length})</h2>
      <table><thead><tr><th>Identificador</th><th>Apellidos</th><th>Nombres</th><th>Seccion</th></tr></thead>
      <tbody>${notVoted.map(v=>`<tr><td>${sanitize(v.identificador)}</td><td>${sanitize(v.apellidos)}</td><td>${sanitize(v.nombres)}</td><td>${sanitize(getSecNombre(v))}</td></tr>`).join('')||'<tr><td colspan="4" style="text-align:center">Sin registros</td></tr>'}
      </tbody></table>`;
    await exportToPDF('Participacion - '+election.nombre,html);
  }catch(err){showToast('Error: '+err.message,'error');}
  finally{hideLoading();}
}
      ...
      <tbody>${sections.length===0?'<tr><td colspan="5" style="text-align:center;padding:32px;color:var(--text-light)">No hay secciones</td></tr>':
        sections.map(s=>{
          ...
          return `<tr>
            ...
            <td><div style="display:flex;gap:6px;flex-wrap:wrap">
              <button class="btn btn-ghost btn-sm" onclick="openSectionModal('${s.id}')" title="Editar">✏️</button>
              <button class="btn btn-outline btn-sm" onclick="openAssignSectionModal('${s.id}')" title="Asignar a votación">➕ Votación</button>
              <button class="btn btn-danger btn-sm" onclick="deleteSectionAdmin('${s.id}')" title="Eliminar">🗑️</button>
            </div></td></tr>`;
        }).join('')}
      </tbody></table></div>`;
}
...
// Modal para asignar sección a votación adicional (sin quitar las anteriores)
function openAssignSectionModal(id) {
  const s=(AppState.sections||[]).find(x=>x.id===id); if(!s) return;
  const currentIds=s.eleccionIds||[];
  // Mostrar solo votaciones que aún no tiene asignadas
  const available=(AppState.elections||[]).filter(e=>!currentIds.includes(e.id));
  if(available.length===0){
    showToast('Esta sección ya está asignada a todas las votaciones existentes','info');
    return;
  }
  const elecOpts=available.map(e=>`<option value="${e.id}">${sanitize(e.nombre)}</option>`).join('');
  const m=document.createElement('div');m.className='modal-overlay';m.id='assign-section-modal';
  m.innerHTML=`<div class="modal-box" style="max-width:420px">
    <div class="modal-header">
      <h3 class="modal-title">➕ Asignar sección a votación</h3>
      <button class="modal-close" onclick="el('assign-section-modal').remove()">✕</button>
    </div>
    <div class="modal-body">
      <p style="color:var(--text-secondary);font-size:.88rem;margin-bottom:16px">
        Sección: <strong>${sanitize(s.nombre)}</strong><br>
        Votaciones actuales: ${currentIds.length>0?currentIds.map(eid=>{const e=(AppState.elections||[]).find(x=>x.id===eid);return e?sanitize(e.nombre):'?';}).join(', '):'Ninguna'}
      </p>
      <div class="form-group">
        <label class="form-label">Agregar a esta votación:</label>
        <select id="assign-eleccion" class="form-input form-select">
          <option value="">— Selecciona —</option>${elecOpts}
        </select>
      </div>
      <div id="assign-error" class="alert alert-error" style="display:none"></div>
    </div>
    <div class="modal-footer">
      <button class="btn btn-ghost" onclick="el('assign-section-modal').remove()">Cancelar</button>
      <button class="btn btn-primary" onclick="doAssignSection('${id}')">➕ Asignar</button>
    </div>
  </div>`;
  document.body.appendChild(m);
  m.onclick=ev=>{if(ev.target===m)m.remove();};
...
async function doAssignSection(id) {
  const newElecId=el('assign-eleccion').value;
  const errEl=el('assign-error');
  if(!newElecId){errEl.textContent='Selecciona una votación';errEl.style.display='flex';return;}
  const s=(AppState.sections||[]).find(x=>x.id===id); if(!s) return;
  const newIds=[...(s.eleccionIds||[]),newElecId];
  showLoading('Asignando...');
  try{
    const res=await API.saveSection({id,nombre:s.nombre,tipo:s.tipo,eleccionIds:newIds});
    if(res.ok){
      const fresh=await API.getSections(); if(fresh.ok) AppState.sections=fresh.data;
      el('assign-section-modal')?.remove();
      renderSectionsMgmt();
      showToast('Sección asignada a la nueva votación','success');
    } else {errEl.textContent=res.error||'Error';errEl.style.display='flex';}
  }catch(err){errEl.textContent='Error: '+err.message;errEl.style.display='flex';}
  finally{hideLoading();}
}
...
function openUrnaModal(id=null) {
  ...
  // Filtrar ubicaciones por votación actual de la urna
  const eleccionActualId2 = u?.eleccionId||'';
  function buildLocOpts(eId) {
    const locsFiltradas = eId
      ? locations.filter(l => !l.eleccionId || l.eleccionId === eId)
      : locations;
    return locsFiltradas.map(l=>`<option value="${sanitize(l.nombre)}" ${u&&u.ubicacion===l.nombre?'selected':''}>${sanitize(l.nombre)}</option>`).join('');
  }
  const locOpts = buildLocOpts(eleccionActualId2);
  ...
  const permsActuales=u?.seccionesPermitidas?u.seccionesPermitidas.split(',').filter(Boolean):[];
  // Calcular secciones disponibles según la votación actual de la urna
  const eleccionActualId=u?.eleccionId||'';
  function buildSectionOpts(eId,perms) {
    const secsFiltradas = eId
      ? sections.filter(s=>(s.eleccionIds||[]).includes(eId))
      : sections;
    if(secsFiltradas.length===0) return '<option disabled>Sin secciones para esta votación</option>';
    return secsFiltradas.map(s=>`<option value="${s.id}" ${perms.includes(s.id)?'selected':''}>${sanitize(s.nombre)}</option>`).join('');
  }
  const sectionOpts=buildSectionOpts(eleccionActualId,permsActuales);
  ...
  m.innerHTML=`<div class="modal-box">
    ...
    <div class="modal-body">
      <div class="form-group"><label class="form-label">¿Para qué votación es esta urna? *</label>
        <select id="urna-eleccion" class="form-input form-select" onchange="updateUrnaSeccionesByEleccion(this.value); updateUrnaUbicacionByEleccion(this.value)">
          <option value="">— Selecciona una votación —</option>${elecOpts}
        </select></div>
...
// Actualizar ubicaciones en modal urna cuando cambia la votación
function updateUrnaUbicacionByEleccion(eleccionId) {
  const locations = AppState.locations || [];
  const locsFiltradas = eleccionId
    ? locations.filter(l => !l.eleccionId || l.eleccionId === eleccionId)
    : locations;
  const sel = el('urna-ubicacion');
  if (!sel) return;
  if (locsFiltradas.length === 0) {
    sel.innerHTML = '<option value="">Sin ubicaciones para esta votación</option>';
  } else {
    sel.innerHTML = '<option value="">Seleccionar...</option>' +
      locsFiltradas.map(l => `<option value="${sanitize(l.nombre)}">${sanitize(l.nombre)}</option>`).join('');
  }
}

// Actualizar lista de secciones en modal urna cuando cambia la votación
function updateUrnaSeccionesByEleccion(eleccionId) {
  const sections = AppState.sections || [];
  const secsFiltradas = eleccionId
    ? sections.filter(s => (s.eleccionIds||[]).includes(eleccionId))
    : sections;
  const sel = el('urna-secciones');
  if (!sel) return;
  if (secsFiltradas.length === 0) {
    sel.innerHTML = '<option disabled>Sin secciones asignadas a esta votación</option>';
    sel.style.opacity = '0.5';
  } else {
    sel.innerHTML = secsFiltradas.map(s => `<option value="${s.id}">${sanitize(s.nombre)}</option>`).join('');
    sel.style.opacity = '1';
  }
}
